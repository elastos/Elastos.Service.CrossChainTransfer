...
public class LoginService {
    ...
    public String userReg(String email, String name, String password) {
        if(StringUtils.isAnyBlank(email, name, password)){
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("传入参数不能为空").toJsonString();
        }
        ...
        if (ret.isPresent()) {
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("邮箱已占用").toJsonString();
        }
        ...
        if (null == user.getId()) {
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_DATABASE).setMsg("服务器内部错误").toJsonString();
        }

        return new ServerResponse().setState(ServerResponseCode.SUCCESS).setData("注册成功").toJsonString();
    }
    ...
    public String userLogin(HttpSession session, String email, String password) {
        ...
        if (!ret.isPresent()) {
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_DATA_NOT_FOUND).setMsg("邮箱未注册").toJsonString();
        }
        ...
        if(!encodedPassword.equals(user.getPassword())){
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("密码错误").toJsonString();
        }
    ...
    public String getUserInfo (Long uid) {
        ...
        if(!ret.isPresent()){
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_DATA_NOT_FOUND).setMsg("用户未注册").toJsonString();
        }
    ...
    public String retrievePassword (HttpSession session, String email) {
        ...
        if (!ret.isPresent()) {
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("邮箱未注册").toJsonString();
        }
...
//                            code)){
        try {
            if(!SendEmail.sendByAmazon(eMailConf.getFrom(), email, "The code is:"+code,
                ...
                return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("发送邮件失败，请稍候再试").toJsonString();
            }
        } catch (Exception e){
            logger.error("retrievePassword send mail exception.");
            System.out.println("retrievePassword send mail exception.");
            return new ServerResponse().setState(ServerResponseCode.ERROR_INTERNAL).setMsg("发送邮件失败，请稍候再试").toJsonString();
        }

        return new ServerResponse().setState(ServerResponseCode.SUCCESS).setData("code:"+code).toJsonString();
    }
    ...
    public String updateRetrievePassword (HttpSession session, String code, String password) {
        ...
        if(!VerificationUtil.isCodeCorrect(session, code)){
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("验证码错误").toJsonString();
        }
        ...
        if(!VerificationUtil.isNotExpired(session)){
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("验证码已过期,请重新发送").toJsonString();
        }
        ...
        if (null == email) {
            ...
            return new ServerResponse().setState(ServerResponseCode.ERROR_PARAMETER).setMsg("邮箱地址错误").toJsonString();
        }
